...
So \(\det(A_0-\lambda)\) is a factor of \(\det(A-\lambda)\). Now \((A_0-\lambda)x=(\lambda_0-\lambda)x\) for all \(x\in M\), so \(\det(A_0-\lambda)=(\lambda_0-\lambda)^m\), where \(m=\dim M\) is the geometric multiplicity of~\(\lambda_0\), by the determinant of scalar maps (\S~53, p.~99). It follows that the algebraic multiplicity of~\(\lambda_0\) as a root of~\(\det(A-\lambda)\) is at least~\(m\).
\end{rmk}
