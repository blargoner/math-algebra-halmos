...
\end{bwd}
\item[(b)]
Geometrically, this is evident from~(a) because two vectors in x-y-z space are linearly dependent if and only if their corresponding projections in each of the x-y, x-z, and y-z planes are linearly dependent. Formally:
\begin{fwd}
...
If \(x_i=0\), then \(y_i=0\) and the result follows from~(a) applied to the other coordinates.\qedhere
\end{bwd}
\end{enumerate}
\end{proof}
\section*{Chapter~II}
...
\end{exer}
\begin{proof}
Recall for a projection~\(P\) on~\(V\), \(V=\ran(P)\dsum\ker(P)\) and (\S~41, Theorem~2)
